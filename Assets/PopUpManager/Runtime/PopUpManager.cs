...
public class PopUpManager : ScriptableObject
{
    ...
    public static void Show<TB,T>(TB prefab, T config) where TB : GenericPopUp<T> where T : PopUp.GenericPopUpConfig<T>
    {
        Show(prefab.gameObject, config);
    }
    ...
    public static void Enqueue<TB,T>(TB prefab, T config) where TB : GenericPopUp<T> where T : PopUp.BasePopUpConfig
    {
        Enqueue(prefab.gameObject, config);
    }
